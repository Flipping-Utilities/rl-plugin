...
public class JsonToSqliteParityTest {
    ...
    public void testPartialConsumedTrades() {
        ...
        // Check stats from the repository (events-based, not trade-based)
        FlipRepository repository = new SqliteFlipRepository(storage, null);
        AggregateStats repoStats = repository.getAggregateStats(TEST_ACCOUNT, Instant.EPOCH);

        // Profit from the flip event
        assertEquals("Profit should be 50000", 50000L, repoStats.totalProfit);

        // Expense/revenue from the flip event (cost=500000, profit=50000, so revenue=550000).
        // These are events-based totals, not unconsumed-trade totals — the repository sources
        // from the events table, not from the trades+consumed_trade CTE.
        assertEquals("Event cost should be 500000", 500000L, repoStats.totalExpense);
        assertEquals("Event revenue (cost+profit) should be 550000", 550000L, repoStats.totalRevenue);
    }
}
