...
public class MigrationRollbackTest {
    ...
    /**
     * Test 4: Verify migration is idempotent - inserting the same trade (same account+uuid)
     * via the migration path twice does not duplicate data. This works because Phase 1 added
     * UNIQUE(account_id, uuid) and Phase 3 switched to INSERT OR IGNORE.
     */
    @Test
    public void testMigrationIdempotent() throws Exception {
        String displayName = "IdempotentPlayer";

        sqliteStorage.upsertAccount(displayName, null);

        // Insert a trade with a known uuid
        long ts = 1700000000000L;
        sqliteStorage.insertTrade(displayName, 4151, "uuid-idempotent-1", ts, 10, 50000, true);

        List<Map<String, Object>> trades1 = sqliteStorage.loadTrades(displayName, Instant.EPOCH);
        int countAfterFirstInsert = trades1.size();
        assertEquals("Should have 1 trade after first insert", 1, countAfterFirstInsert);

        // "Re-insert" the same trade (same uuid) — should be ignored by INSERT OR IGNORE
        // against UNIQUE(account_id, uuid).
        sqliteStorage.insertTrade(displayName, 4151, "uuid-idempotent-1", ts, 10, 50000, true);

        List<Map<String, Object>> trades2 = sqliteStorage.loadTrades(displayName, Instant.EPOCH);
        assertEquals("Re-inserting the same trade (same uuid) should NOT duplicate it",
            countAfterFirstInsert, trades2.size());
    }
