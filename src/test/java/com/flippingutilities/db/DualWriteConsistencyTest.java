...
public class DualWriteConsistencyTest {
    ...
    /**
     * Test 6b: Recording with a uuid and an explicit tax amount must persist the caller's
     * tax (not a recomputed one), and re-recording the same uuid must not duplicate the trade.
     */
    @Test
    public void testRecordTradeWithUuidAndExplicitTax() {
        SqliteFlipRepository repository = new SqliteFlipRepository(storage, null);

        String displayName = "ExplicitTaxPlayer";
        storage.upsertAccount(displayName, null);

        long timestamp = Instant.now().toEpochMilli();
        // Tax for a 1000gp sale today would be floor(1000 * 0.02) * 10 = 200;
        // pass 1234 to prove the caller-supplied value wins.
        repository.recordTrade(displayName, 4151, "tax-uuid", timestamp, 10, 1000, false, 1234);
        // Re-record the same offer (e.g. duplicate event): must be ignored, not duplicated.
        repository.recordTrade(displayName, 4151, "tax-uuid", timestamp, 10, 1000, false, 1234);

        List<FlipRepository.TradeRecord> trades = repository.getTradesForItem(displayName, 4151, Instant.EPOCH);
        assertEquals("Duplicate uuid should not create a second trade", 1, trades.size());

        Map<String, Object> stats = storage.queryAggregateStats(displayName, Instant.EPOCH);
        assertEquals("Caller-supplied tax must be persisted as-is", 1234L, stats.get("taxPaid"));
    }
