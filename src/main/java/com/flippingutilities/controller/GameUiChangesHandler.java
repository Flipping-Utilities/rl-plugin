...
public class GameUiChangesHandler {
    ...
    public void onVarClientIntChanged(VarClientIntChanged event) {
        ...
        plugin.getClientThread().invokeLater(() ->
        {
            ...
            Optional<FlippingItem> selectedItem = plugin.viewItemsForCurrentView().stream().filter(item -> item.getItemId() == client.getVarcIntValue(CURRENT_GE_ITEM)).findFirst();
            ...
            if (chatInputText.equals("How many do you wish to buy?")) {
                ...
                if (!selectedItem.isPresent()) {
                    ItemStats itemStats = plugin.getItemManager().getItemStats(client.getVarcIntValue(CURRENT_GE_ITEM), false);
    ...
    public void onVarbitChanged(VarbitChanged event) {
        ...
        if (event.getIndex() == CURRENT_GE_ITEM.getId() && client.getVarcIntValue(CURRENT_GE_ITEM) != -1 && client.getVarcIntValue(CURRENT_GE_ITEM) != 0) {
            highlightOffer(plugin.getClient().getVarcIntValue(CURRENT_GE_ITEM));
        }
        ...
        if (event.getIndex() == CURRENT_GE_ITEM.getId() &&
                (client.getVarcIntValue(CURRENT_GE_ITEM) == -1 || client.getVarcIntValue(CURRENT_GE_ITEM) == 0) && highlightedItem.isPresent()) {
            deHighlightOffer();
        }
    }
