...
1. Reconcile the complete pre-`T` history first, including sales and recipe consumption before `P`. The cutoff filters remaining opening candidates after reconciliation; it must not erase sales or reset a partially consumed purchase to its original quantity. For a fresh start, use the deterministic new matching policy to derive residual candidates; earlier legacy reports remain archived and are never combined with the new ledger.
2. Hybrid carry-in has a stronger condition: its source segments must be unused by **every supported frozen legacy report**, not just the all-history summary and flip list. For the first implementation, permit hybrid carry-in only for items with no sold quantity anywhere in the frozen legacy source view and for purchase segments unclaimed by frozen recipes. Exclude other items as ambiguous; this conservative rule also covers legacy matching against later purchases. Quantity confirmation cannot override this accounting restriction. A wholly new-side recipe may carry eligible inputs only as an explicitly selected reservation bundle subject to these same checks. Users wanting broader carry-in can choose a fresh ledger or full historical recalculation. Do not introduce another speculative legacy allocator to widen eligibility.
3. All candidates require known source identity and supported cost and must not be deleted or reserved to a conflicting recipe. Show them grouped by item with purchase dates, available quantity, estimated/observed cost, and reasons for exclusion. The player confirms, reduces, or excludes quantities they still own; defaulting to every unmatched historical buy would recreate the stale-inventory problem. Never increase a source-backed quantity above its validated remainder or silently create a zero-cost opening lot.
...
| Buy 10 at 100 five days before `T`; sell 6 before `T`; start fresh with a 30-day purchase cutoff | At most 4 are opening candidates, at total cost 400. If the player confirms only 2, opening cost is 200. Hybrid mode conservatively excludes this item because it has legacy sales. |
...
| Buy A: 2 at 100 on day 1; buy B: 2 at 110 on day 2; sell 2 at 150 on day 3; hybrid cutover on day 4 | All-history calculations leave B unused, but a legacy period starting before B spends its 220 basis. B is ineligible for hybrid carry-in. A fresh ledger can review the new-policy remainder independently, without combining archived legacy totals. |
...
| Opening inventory | No carry-in; purchases exactly at `P`/`T`; fixed 7/30/90-day cutoffs; old sales applied before candidate filtering; partial remainder and quantity reduction; all-history leftovers consumed by narrower legacy reports; hybrid no-legacy-sales restriction; unknown dates/cost; old consumed inventory excluded; no rolling expiry or manufactured zero cost. |
